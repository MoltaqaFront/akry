  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        await this.$axios({
          method: "POST",
          url: `admin/nationalities/${this.$route.params.id}`,
          data: REQUEST_DATA,
        });
        this.isWaitingRequest = false;
    ...
    async getNationalityData() {
      try {
        ...
        this.data.nameAr = res.data.body.nationality.translations.title.ar;
        this.data.nameEn = res.data.body.nationality.translations.title.en;
        this.data.is_active = res.data.body.nationality.is_active;
