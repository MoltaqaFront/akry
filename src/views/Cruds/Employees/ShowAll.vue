    ...
    <main>
        ...
        <div class="filter_form_wrapper">
          <form @submit.prevent="submitFilterForm">
            <div class="row justify-content-center align-items-center w-100">
              ...
              <!-- Start:: Name Input -->
              <base-input col="3" type="text" :placeholder="$t('PLACEHOLDERS.store_name')"
                v-model.trim="filterOptions.store_name" />
              <!-- End:: Name Input -->
              ...
              <!-- Start:: Name Input -->
              <base-input col="3" type="tel" :placeholder="$t('PLACEHOLDERS.mobileNumber')"
      ...
      <div class="table_title_wrapper">
        <div class="title_text_wrapper">
          <h5>{{ $t("PLACEHOLDERS.Store_employees") }}</h5>
      ...
      <v-data-table class="thumb" :loading="loading" :loading-text="$t('TABLES.loadingData')" :search="searchValue"
        ...
        <template v-slot:[`item.store_name`]="{ item }">
          <p class="text-danger" v-if="!item.store.title"> {{ $t("TABLES.noData") }} </p>
        ...
        <template v-slot:[`item.email`]="{ item }">
          <p class="text-danger" v-if="!item.email"> {{ $t("TABLES.noData") }} </p>
        ...
        <template v-slot:[`item.phone`]="{ item }">
          <p class="text-danger" v-if="!item.phone"> {{ $t("TABLES.noData") }} </p>
  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== '1') {
        await this.$router.push({ path: '/employees/all', query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
      ...
      try {
        let res = await this.$axios({
          ...
          params: {
            ...
            store_name: this.filterOptions.store_name,
        ...
        this.loading = false;
        this.tableRows = res.data.body.employees;
    ...
    async changeActivationStatus(item) {
      try {
        await this.$axios({
          method: "PUT",
          url: `admin/employees/${item.id}/activate`,
        });
    ...
    showItem(item) {
      this.$router.push({ path: `/employees/show/${item.id}` });
    },
    // ===== End:: End
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `admin/employees/${this.itemToDelete.id}`,
        });
