    ...
    <main>
      ...
      <div class="table_title_wrapper">
        <div class="title_text_wrapper">
          <h5>{{ $t("PLACEHOLDERS.car_modals") }}</h5>
  ...
  data() {
    return {
      ...
      tableHeaders: [
        ...
        {
          text: this.$t("PLACEHOLDERS.car_modal"),
          value: "title",
          align: "center",
        },
        {
          text: this.$t("PLACEHOLDERS.car_type"),
          value: "car_type.name",
          align: "center",
        },
        {
  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== '1') {
        await this.$router.push({ path: '/carModals/all', query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
      ...
      this.filterOptions.carType = null;
      if (this.$route.query.page !== '1') {
        await this.$router.push({ path: '/carModals/all', query: { page: 1 } });
      }
      this.setTableRows();
      ...
      try {
        let res = await this.$axios({
          ...
          params: {
            ...
            car_type_id: this.filterOptions.carType?.id,
          },
        });
        this.loading = false;
        this.tableRows = res.data.body.carModel;
    ...
    async changeActivationStatus(item) {
      try {
        await this.$axios({
          method: "PUT",
          url: `admin/carModels/${item.id}/activate`,
        });
    ...
    // ===== Start:: End
    editItem(item) {
      this.$router.push({ path: `/carModals/edit/${item.id}` });
    },

    showItem(item) {
      this.$router.push({ path: `/carModals/show/${item.id}` });
    },
    // ===== End:: End
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `admin/carModels/${this.itemToDelete.id}`,
        });
    ...
    async getAllCarType() {
      ...
      try {
        ...
        this.allTypes = res.data.body.CarTypes;
