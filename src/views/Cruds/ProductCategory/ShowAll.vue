    ...
    <main>
        ...
        <div class="filter_form_wrapper">
          <form @submit.prevent="submitFilterForm">
            <div class="row justify-content-center align-items-center w-100">
              ...
              <!-- Start:: Name Input -->
              <base-input col="4" type="text" :placeholder="$t('PLACEHOLDERS.section')"
                v-model.trim="filterOptions.title" />
              <!-- End:: Name Input -->

              <!-- Start:: Store Category Input -->
              <base-select-input col="4" :optionsList="allStores" :placeholder="$t('PLACEHOLDERS.store_name')"
                v-model="filterOptions.store_name" required />
              <!-- multiple -->
      ...
      <div class="table_title_wrapper">
        ...
        <div class="title_route_wrapper" v-if="permissions.create">
          <router-link to="/product_category/create">
      ...
      <v-data-table class="thumb" :loading="loading" :loading-text="$t('TABLES.loadingData')" :search="searchValue"
        ...
        <template v-slot:[`item.store_name`]="{ item }">
          <p class="text-danger" v-if="item.store === null"> {{ $t("TABLES.noData") }} </p>
          <p v-else>{{ item.store.name }}</p>
        </template>
        <!-- End:: Title -->

        <!-- Start:: Title -->
        <template v-slot:[`item.created_at`]="{ item }">
          <p class="text-danger" v-if="!item.created_at"> {{ $t("TABLES.noData") }} </p>
  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== '1') {
        await this.$router.push({ path: '/product_category/all', query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
      this.filterOptions.title = null;
      this.filterOptions.store_name = null;
      ...
      try {
        let res = await this.$axios({
          ...
          params: {
            ...
            title: this.filterOptions.title,
            store_id: this.filterOptions.store_name?.id,
            is_active: this.filterOptions.is_active?.value,
          },
        });
        this.loading = false;
        this.tableRows = res.data.body.product_category;
    ...
    async changeActivationStatus(item) {
      try {
        await this.$axios({
          method: "PUT",
          url: `admin/product_categories/${item.id}/activate`,
        });
    ...
    // ===== Start:: End
    editItem(item) {
      this.$router.push({ path: `/product_category/edit/${item.id}` });
    },

    showItem(item) {
      this.$router.push({ path: `/product_category/show/${item.id}` });
    },
    // ===== End:: End
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `admin/product_categories/${this.itemToDelete.id}`,
        });
