      ...
      <form @submit.prevent="validateFormInputs">
        <div class="row">
          ...
          <base-image-upload-input col="3" identifier="store_logo" :preSelectedImage="data.logo.path"
            :placeholder="$t('PLACEHOLDERS.logo')" @selectImage="selectImage" disabled />
          <!-- End:: Logo Input -->

          <!-- Start:: Cover Input -->
          <base-image-upload-input col="3" identifier="store_cover" :placeholder="$t('PLACEHOLDERS.cover')"
            @selectImage="selectImage" :preSelectedImage="data.cover.path" disabled />
          <!-- End:: Cover Input -->
          ...
          <base-image-upload-input col="3" identifier="commercial_profile_image"
            ...
            :preSelectedImage="data.commericalProfile.path" disabled />
          ...
          <base-image-upload-input col="3" identifier="tax_profile_image"
            ...
            :preSelectedImage="data.taxProfile.path" disabled />
          <!-- End:: Tax Profile Image Input -->

          <!-- Start:: Name Input -->
          <base-input col="4" type="text" :placeholder="$t('PLACEHOLDERS.name')" v-model.trim="data.name" disabled />
          <!-- End:: Name Input -->

          <!-- Start:: Phone Input -->
          <base-input col="4" type="tel" :placeholder="$t('PLACEHOLDERS.phone')" v-model.trim="data.phone" disabled />
          <!-- End:: Phone Input -->

          <!-- Start:: Email Input -->
          <base-input col="4" type="email" :placeholder="$t('PLACEHOLDERS.email')" v-model.trim="data.email" disabled />
          <!-- End:: Email Input -->
          ...
          <base-input col="4" v-if="isSelected(4)" type="text"
            :placeholder="$t('PLACEHOLDERS.starting_price_reservation')" v-model.trim="data.min_charge" disabled />

          <base-select-input v-if="isSelected(4)" col="4" :optionsList="tableType"
            :placeholder="$t('PLACEHOLDERS.table_type')" v-model="data.table_type" multiple disabled />
          <!-- End:: Store Category Input -->

          <base-select-input v-if="isSelected(4)" col="4" :optionsList="sessionType"
            :placeholder="$t('PLACEHOLDERS.session_type')" v-model="data.session_type" disabled />

          <base-select-input v-if="isSelected(4)" col="4" :optionsList="allowKids"
            :placeholder="$t('PLACEHOLDERS.Children_allowed')" v-model="data.allow_kids" disabled />
          ...
          <!-- <base-input col="6" type="password" :placeholder="$t('PLACEHOLDERS.confirmPassword')"
            v-model.trim="data.passwordConfirmation" required disabled /> -->
          ...
          <v-container class="px-0 custom_labels">
            <div class="form-group" v-for="(item, index) in days" :key="'w' + index">
              <input type="checkbox" :value="item" :id="item.title" v-model="selectedDays" />
          ...
          <div class="row">
            ...
            <div class="col-12">
              <div class="all_row_input" v-for="(day, index) in selectedDays" :key="index"
                v-if="day.working_hours && day.working_hours.length > 0">
                <div class="hours_dates" v-for="(workingHour, workingHourIndex) in day.working_hours"
                  :key="'wh' + workingHourIndex">
                  <div class="form-group position-relative">
                    <label class="form-label">{{ $t('TABLES.ImagesSpaces.start_time') }}</label>
                    <input v-model="workingHour.from" type="time" disabled />
                  </div>
                  <div class="form-group position-relative">
                    <label class="form-label">{{ $t('TABLES.ImagesSpaces.end_time') }}</label>
                    <input v-model="workingHour.to" type="time" disabled />
                  </div>
                </div>
                <!-- <div class="all_action">
                  <span @click="addInput(day)" class="add_another">
                    <i class="fas fa-plus"></i>
                  </span>
                </div> -->
                <!-- <div class="inpts" v-for="(input, inputIndex) in day.additionalInputs" :key="'input' + inputIndex">
                  <div class="form-group position-relative">
                    <label class="form-label">{{ $t('TABLES.ImagesSpaces.start_time') }}</label>
                    <input v-model="input.from" type="time" @input="data.from.push($event.target.value)" />
                  </div>
                  <div class="form-group position-relative">
                    <label class="form-label">{{ $t('TABLES.ImagesSpaces.end_time') }}</label>
                    <input v-model="input.to" type="time" @input="data.to.push($event.target.value)" />
                  </div>
                  <div class="all_action">
                    <span @click="removeInput(day, inputIndex)" class="add_another">
