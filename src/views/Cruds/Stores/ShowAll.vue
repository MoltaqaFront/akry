    ...
    <main>
        ...
        <div class="filter_form_wrapper">
          <form @submit.prevent="submitFilterForm">
            <div class="row justify-content-center align-items-center w-100">
              <!-- Start:: Name Input -->
              <base-input col="4" type="text" :placeholder="$t('PLACEHOLDERS.store_name')"
                v-model.trim="filterOptions.name" />

              <base-input col="4" type="text" :placeholder="$t('PLACEHOLDERS.name_provider')"
                v-model.trim="filterOptions.name_provider" />

              <base-input col="4" type="tel" :placeholder="$t('PLACEHOLDERS.mobileNumber')"
              ...
              <!-- <base-select-input col="4" :optionsList="allStores" :placeholder="$t('PLACEHOLDERS.storeType')"
      ...
      <v-data-table class="thumb" :loading="loading" :loading-text="$t('TABLES.loadingData')" :search="searchValue"
        ...
        <template v-slot:[`item.image`]="{ item }">
          <div class="table_image_wrapper">
            ...
            <button class="my-1" @click="showImageModal(item.logo)" v-else>
              <img class="rounded" :src="item.logo" :alt="item.title" width="60" height="60" />
            </button>
          </div>
        </template>
        <!-- End:: Item Image -->
        ...
        <template v-slot:[`item.owner_name`]="{ item }">
          <p class="text-danger" v-if="!item.owner_name || item.owner_name == ''"> {{ $t("TABLES.noData") }} </p>
          <p v-else>{{ item.owner_name }}</p>
        </template>

        <template v-slot:[`item.phone`]="{ item }">
          <p class="text-danger" v-if="!item.owner_phone"> {{ $t("TABLES.noData") }} </p>
          <p v-else>{{ item.owner_phone }}</p>
        </template>
        <!-- End:: Title -->

        <!-- Start:: Description -->
        <template v-slot:[`item.order_types`]="{ item }">
          <h6 class="text-danger" v-if="item.order_types.length === 0"> {{ $t("TABLES.noData") }} </h6>

          <div class="actions" v-else>
            <v-chip color="secondary" v-for="(inner_item, index) in item.order_types" :key="index">
              {{ inner_item.title }}
            </v-chip>
          </div>
        </template>
        ...
        <template v-slot:top>
          ...
          <v-dialog v-model="changeStatus">
            <v-card>
              <v-card-title class="text-h5 justify-center w-100" v-if="itemToChangeActivationStatus">
                {{ $t("PLACEHOLDERS.change_status", { name: itemToChangeActivationStatus.id }) }}
                ...
                <div class="filter_form_wrapper w-100" v-if="allStatus && status_modal.id == 2">
                  <base-input col="12" rows="9" type="textarea" :placeholder="$t('PLACEHOLDERS.deactivateReason')"
              ...
              <v-card-actions>
                ...
                <v-btn class="modal_cancel_btn" @click="changeStatus = false">{{ $t("BUTTONS.cancel") }}</v-btn>
                <v-spacer></v-spacer>
              </v-card-actions>
            </v-card>
          </v-dialog>
  ...
  data() {
    return {
      ...
      tableHeaders: [
        ...
        {
          text: this.$t("TABLES.StoresTypes.image"),
          value: "image",
          align: "center",
        },
        {
        ...
        {
          text: this.$t("PLACEHOLDERS.orderType"),
          value: "order_types",
          align: "center",
        },
  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== '1') {
        await this.$router.push({ path: '/stores/all', query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
      this.filterOptions.name = null;
      this.filterOptions.name_provider = null;
      ...
      try {
        let res = await this.$axios({
          ...
          params: {
            ...
            owner_name: this.filterOptions.name_provider,
            storeType: this.filterOptions.storeType?.id,
            order_type: this.filterOptions.orderType?.id,
            owner_phone: this.filterOptions.phone,
            is_active: this.filterOptions.is_active?.value,
          },
        });
        this.loading = false;
        // console.log("All Data ==>", res.data.data);
        this.tableRows = res.data.body.stores;
    ...
    async changeActivationStatus(item) {
      try {
        await this.$axios({
          method: "PUT",
          url: `admin/stores/${item.id}/activate`,
        });
    ...
    async confirmChangeStatus() {
      try {
        ...
        if (this.reason) {
          var reason_pop = this.reason
        }

        await this.$axios({
          method: "PUT",
          url: `admin/get-stores/${this.itemToChangeActivationStatus.id}`,
          data: { is_approved: this.status_modal.value, reason: reason_pop }
        });

        this.changeStatus = false;
        this.tableRows = this.tableRows.filter((item) => item.id !== this.itemToChangeActivationStatus.id);
        this.setTableRows();
    ...
    // ===== Start:: End
    editItem(item) {
      this.$router.push({ path: `/stores/edit/${item.id}` });
    },

    showItem(item) {
      this.$router.push({ path: `/stores/show/${item.id}` });
    },
    // ===== End:: End
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `admin/stores/${this.itemToDelete.id}`,
        });
