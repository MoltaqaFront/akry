    ...
    <main>
      ...
      <div class="table_title_wrapper">
        <div class="title_text_wrapper">
          <h5>{{ $t("PLACEHOLDERS.car_types") }}</h5>
        ...
        <div class="title_route_wrapper" v-if="permissions.create">
          <router-link to="/carTypes/create">
            {{ $t("PLACEHOLDERS.add_car_type") }}
          </router-link>
        </div>
      </div>
      <!--  =========== End:: Table Title =========== -->
  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== '1') {
        await this.$router.push({ path: '/carTypes/all', query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
    ...
    async setTableRows() {
      ...
      try {
        ...
        this.loading = false;
        this.tableRows = res.data.body.CarTypes;
    ...
    async changeActivationStatus(item) {
      try {
        await this.$axios({
          method: "PUT",
          url: `admin/carTypes/${item.id}/activate`,
        });
    ...
    // ===== Start:: End
    editItem(item) {
      this.$router.push({ path: `/carTypes/edit/${item.id}` });
    },

    showItem(item) {
      this.$router.push({ path: `/carTypes/show/${item.id}` });
    },
    // ===== End:: End
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `admin/carTypes/${this.itemToDelete.id}`,
        });
