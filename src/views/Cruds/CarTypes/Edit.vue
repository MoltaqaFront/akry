<template>
  <div class="crud_form_wrapper">
    ...
    <div class="form_title_wrapper">
      <h4>{{ $t("PLACEHOLDERS.edit_car_type") }}</h4>
    </div>
    <!-- End:: Title -->
  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        await this.$axios({
          method: "POST",
          url: `admin/carTypes/${this.$route.params.id}`,
          data: REQUEST_DATA,
        });
        this.isWaitingRequest = false;
        this.$message.success(this.$t("MESSAGES.addedSuccessfully"));
        this.$router.push({ path: "/carTypes/all" });
      } catch (error) {
        this.isWaitingRequest = false;
    ...
    async getCarTypes() {
      try {
        ...
        this.data.nameAr = res.data.body.car_type.translations.title.ar;
        this.data.nameEn = res.data.body.car_type.translations.title.en;
        this.data.is_active = res.data.body.car_type.is_active;
