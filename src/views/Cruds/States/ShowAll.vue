    ...
    <main>
        ...
        <div class="filter_form_wrapper">
          <form @submit.prevent="submitFilterForm">
            <div class="row justify-content-center align-items-center w-100">
              ...
              <!-- Start:: Name Input -->
              <base-input col="6" type="text" :placeholder="$t('PLACEHOLDERS.state')"
      ...
      <v-data-table class="thumb" :loading="loading" :loading-text="$t('TABLES.loadingData')" :search="searchValue"
        ...
        <template v-slot:[`item.title`]="{ item }">
          <p class="text-danger" v-if="!item.state.title"> {{ $t("TABLES.noData") }} </p>
          <p v-else>{{ item.state.title }}</p>
        </template>

        <template v-slot:[`item.radius`]="{ item }">
          <p class="text-danger" v-if="!item.radius"> {{ $t("TABLES.noData") }} </p>
          <p v-else>{{ item.radius }}</p>
        </template>
        <!-- End:: Title -->
          ...
          <v-dialog v-model="dialogDelete">
            <v-card>
              <v-card-title class="text-h5 justify-center" v-if="itemToDelete">
                {{ $t("TITLES.DeleteConfirmingMessage", { name: itemToDelete.state.title }) }}
              </v-card-title>
              <v-card-actions>
  ...
  data() {
    return {
      ...
      tableHeaders: [
        ...
        {
          text: this.$t("PLACEHOLDERS.radius"),
          value: "radius",
          align: "center",
        },
  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== '1') {
        await this.$router.push({ path: '/states/all', query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
    ...
    async setTableRows() {
      ...
      try {
        ...
        this.loading = false;
        this.tableRows = res.data.body.regions;
    ...
    async changeActivationStatus(item) {
      try {
        await this.$axios({
          method: "PUT",
          url: `admin/regions/${item.id}/activate`,
        });
    ...
    // ===== Start:: End
    editItem(item) {
      this.$router.push({ path: `/states/edit/${item.id}` });
    },

    showItem(item) {
      this.$router.push({ path: `/states/show/${item.id}` });
    },
    // ===== End:: End
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `admin/regions/${this.itemToDelete.id}`,
        });
