    ...
    <main>
        ...
        <div class="filter_form_wrapper">
          <form @submit.prevent="submitFilterForm">
            <div class="row justify-content-center align-items-center w-100">
              ...
              <!-- Start:: Name Input -->
              <base-input col="4" type="text" :placeholder="$t('PLACEHOLDERS.occasions_type')"
      ...
      <div class="table_title_wrapper">
        <div class="title_text_wrapper">
          <h5>{{ $t("PLACEHOLDERS.occasions_all") }}</h5>
        ...
        <div class="title_route_wrapper" v-if="permissions.create">
          <router-link to="/occasions/create">
            {{ $t("PLACEHOLDERS.occasions_add") }}
          </router-link>
        </div>
      </div>
      <!--  =========== End:: Table Title =========== -->
      ...
      <v-data-table class="thumb" :loading="loading" :loading-text="$t('TABLES.loadingData')" :search="searchValue"
        ...
        <template v-slot:[`item.created_at`]="{ item }">
          <p class="text-danger" v-if="item.store === null"> {{ $t("TABLES.noData") }} </p>
          <p v-else>{{ item.store.created_at }}</p>
        </template>
  ...
  data() {
    return {
      ...
      tableHeaders: [
        ...
        {
          text: this.$t("PLACEHOLDERS.occasions_type"),
          value: "title",
          align: "center",
        },
        {
  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== '1') {
        await this.$router.push({ path: '/occasions/all', query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
    ...
    async setTableRows() {
      ...
      try {
        ...
        this.loading = false;
        this.tableRows = res.data.body.occasions;
    ...
    async changeActivationStatus(item) {
      try {
        await this.$axios({
          method: "PUT",
          url: `admin/occasions/${item.id}/activate`,
        });
    ...
    // ===== Start:: End
    editItem(item) {
      this.$router.push({ path: `/occasions/edit/${item.id}` });
    },

    showItem(item) {
      this.$router.push({ path: `/occasions/show/${item.id}` });
    },
    // ===== End:: End
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `admin/occasions/${this.itemToDelete.id}`,
        });
