  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== '1') {
        await this.$router.push({ path: '/regions/all', query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
    ...
    async setTableRows() {
      ...
      try {
        ...
        this.loading = false;
        this.tableRows = res.data.body.states;
    ...
    async changeActivationStatus(item) {
      try {
        await this.$axios({
          method: "PUT",
          url: `admin/states/${item.id}/activate`,
        });
    ...
    // ===== Start:: End
    editItem(item) {
      this.$router.push({ path: `/regions/edit/${item.id}` });
    },

    showItem(item) {
      this.$router.push({ path: `/regions/show/${item.id}` });
    },
    // ===== End:: End
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `admin/states/${this.itemToDelete.id}`,
        });
