    ...
    <main>
      ...
      <v-data-table class="thumb" :loading="loading" :loading-text="$t('TABLES.loadingData')" :search="searchValue"
        ...
        <template v-slot:[`item.reply`]="{ item }">
          <template v-if="permissions.reply">
            <h6 class="text-danger" v-if="item.reply.length === 0"> {{ $t("TABLES.noData") }} </h6>
        ...
        <template v-slot:[`item.replied`]="{ item }">
          <v-chip :color="item.replied ? 'green' : 'red'" text-color="white" small>
            <template v-if="item.replied">
              {{ $t("STATUS.replied") }}
            </template>
            <template v-else>
              {{ $t("STATUS.notReplied") }}
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <div class="actions">
            <template v-if="permissions.reply">
              <span class="blue-grey--text text--darken-1" v-if="item.replied">
  ...
  methods: {
      ...
      try {
        let res = await this.$axios({
          ...
          params: {
            ...
            replied: this.filterOptions.status?.value,
          },
        });
        this.loading = false;
        // console.log("All Data ==>", res.data.data);
        this.tableRows = res.data.body.contact_us_messages;
    ...
    async sendReplay() {
      ...
      try {
        await this.$axios({
          method: "PUT",
          url: `admin/contact-us/${this.itemToSendReplay.id}`,
          data: REQUEST_DATA,
        });
