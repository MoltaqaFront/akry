      ...
      <v-data-table class="thumb" :loading="loading" :loading-text="$t('TABLES.loadingData')" :search="searchValue"
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <div class="actions">
            ...
            <a-tooltip placement="bottom" v-if="item.id !== 1 && permissions.edit">
              <template slot="title">
            ...
            <a-tooltip placement="bottom" v-if="item.id !== 1 && permissions.delete">
              <template slot="title">
            ...
            <template v-if="item.id !== 1 && !(permissions.edit && permissions.delete)">
          ...
          <v-dialog v-model="dialogDelete">
            <v-card>
              <v-card-title class="text-h5 justify-center" v-if="itemToDelete">
                {{ $t("TITLES.DeleteConfirmingMessage", { name: itemToDelete.role }) }}
              </v-card-title>
              <v-card-actions>
  ...
  methods: {
    ...
    async setTableRows() {
      ...
      try {
        ...
        this.tableRows = res.data.body.roles_and_permissions;
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `admin/roles/${this.itemToDelete.id}`,
        });
