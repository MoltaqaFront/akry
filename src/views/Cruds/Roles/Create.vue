                    ...
                    <div class="row">
                      <div v-for="item in permission.permissions" :key="item.id" class="col-6">
                        <div class="input_wrapper switch_wrapper my-5">
                          <v-switch color="green" :label="item.name" :value="item.id" v-model="data.permissions"
...
export default {
  ...
  methods: {
    ...
    checkAllSwitches() {
      // Iterate through each permission and set its value to true
    ...
    async submitForm() {
      ...
      this.data.permissions.forEach(element => {
        REQUEST_DATA.append("permissions[]", element);
      });
      // Start:: Append Request Data
  ...
  async created() {
    // Start:: Fire Methods
    await this.$store.dispatch("PermissionsModule/checkRoutePermissions", "roles create");
